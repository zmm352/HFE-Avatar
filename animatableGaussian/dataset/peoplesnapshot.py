...
def read_imgs(dataroot, opt, t_flag, resolution):
    print(str(opt))
    start = opt.start
    end = opt.end + 1
    skip = opt.get("skip", 1)
...
class PeopleSnapshotDataset(torch.utils.data.Dataset):
    def __init__(self, dataroot, max_freq, split, t_flag, opt):
        self.split = split
        self.max_freq = max_freq
        self.camera_params = getCamPara(dataroot, opt)
        self.imgs = read_imgs(
            dataroot, opt, t_flag, (self.camera_params.image_width, self.camera_params.image_height))
        ...
        for split in splits:
            ...
            if split == "train":
                dataset = PeopleSnapshotDataset(
                    opt.dataroot, opt.max_freq, split, 1, opt.get(split))
            else:
                dataset = PeopleSnapshotDataset(
                    opt.dataroot, opt.max_freq, split, 0, opt.get(split))
